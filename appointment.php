...
if (isset($_POST['submit'])) {
    ...
    if (empty($errors)) {
        ...
        $query = mysqli_query($con, "SELECT * FROM tblappointment WHERE AptDate = '$adate' AND AptTime = '$atime'");
...
<script>
    $(document).ready(function() {
        // Disable previously selected dates in the date picker
        function disablePreviousDates() {
            var selectedDate = new Date($("#adate").val());
            $(".appointment_date").datepicker("option", "minDate", selectedDate);
        }
        ...
        // Disable previously selected time slots in the time picker
        function disablePreviousTimeSlots() {
        ...
        // Handle time selection change event
        $("#atime").change(function() {
            disablePreviousTimeSlots();
        });

        // Call the functions to disable initial selections
        disablePreviousDates();
        disablePreviousTimeSlots();
    });
</script>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="services" id="services" required="true" class="form-control">
                                            ...
                                            ?>
                                                <option value="<?php echo $row['ServiceName']; ?>"><?php echo $row['ServiceName']; ?>- Rs. <?php echo $row['Cost']; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
